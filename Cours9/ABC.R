...
hist(ech, xlim=c(0, 5))#long avec seulement c(3,4,6)
#est-ce que les données sont bonne avec le modèle que j'ai choisi, plus mes données sont proche c(1,2,1), plus c'est rapide
...
#plus grand que n <- 12, à déconseillé à cause du temps que r va prendre pour faire le programme. En termes de stats, ça serait bon
n <- 30
#n <- 12
...
#n <- 30 et m <- 30, c'est interminable!!!
...
# expérience avec rho la distance maximum entre les échantillons triés
rho <- function(x, y) max(abs(sort(x) - sort(y)))
eps <- 10
ech <- ABC_melange(m, data_obs, rho, eps)
ech
