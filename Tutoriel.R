...
# Ses éléments sont indexés par deux indices (plutôtt qu'un comme les vecteurs)
mat[1,1]
mat[2,3]
...
# Les matrices (et vecteurs) sont transposés à l'aide de la fonction t()
t(vect)
t(mat)
...
# À COMPLÉTER: apply/sapply

# Les fonctions apply, sappl, lapply, permettre d'appliquer en parallèle une fonction f à tous les éléments, lignes, colonnes, ou autres marges
# d'un vecteur, matrice, tableau, etc.

# apply est la fonction la plus générale. Elle accepte trois arguments: les données sur lesquelles la fonction doit être appliquée,
# la (les) marge(s) auxquelles elle doit s'appliquer, et la fonction
mat <- matrix(1:9, nrow=3, ncol=3)
f <- sum

# La marge 1 correspond aux lignes, et la marge 2 aux colonnes
apply(mat, 1, f)
apply(mat, 2, f)

# On peut aussi fournir deux marges, ce qui pour une matrice a pour effet d'appliquer la fonction à chaque entrée
apply(mat, c(1, 2), f)

# Pour un vecteur, il n'y a qu'une marge. On utilise alors la fonction sapply
vect <- c(2, 3, 5)
g <- function(x) x^2
sapply(vect, g)
